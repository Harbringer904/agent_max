...
| ---- | ----------- | ------------------------------------------------------------------------ | ---------------------- |
| R001 | 400         | `jobSpec` is missing or malformed — must be an object with a non-empty string `field` and a non-empty `criteria` array whose items each have a string `key` and a numeric `weight`. | `routes/search.js`, `routes/results.js` |
...
| R500 | 500         | Unhandled error while processing the request (provider/network failure, unexpected exception, etc.). The real error is logged server-side via `console.error`, never returned to the client. | `routes/search.js`, `routes/results.js` |
